...
<Info>
  **Niveau** : Débutant · **Prérequis** : [Introduction au prompt](/prompt-engineering/introduction-au-prompt) · **Temps de lecture** : ~4 min
</Info>
...
<Accordion title="Quiz : un prompt sans aucune entrée (input) peut-il quand même être efficace ?">
  **Oui.** L'entrée n'est obligatoire que « si applicable », c'est-à-dire lorsque vous avez un texte, un document ou des données concrètes à faire traiter par le modèle. Pour une tâche de pure génération, comme rédiger un e-mail de relance à partir de rien, il n'y a pas de données existantes à fournir : instruction, contexte et format de sortie suffisent largement.
</Accordion>
