...
<Info>
  **Niveau** : Avancé · **Prérequis** : [Analyse et synthèse](/prompt-engineering/analyse-et-synthese) · **Temps de lecture** : ~8 min
</Info>
...
```text
Écris une fonction Python qui prend une liste de prix (float) et
retourne le prix total après application d'une remise de 10% si
le total dépasse 100€, sinon retourne le total sans remise.
Gère le cas d'une liste vide (retourne 0).
```
...
<Accordion title="Quiz : si du code généré par l'IA compile sans erreur et semble fonctionner sur vos premiers tests, peut-on le déployer directement en production ?">
