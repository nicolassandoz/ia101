...
<Note>
  Tout ce que vous venez de lire décrit le **CoT prompté** : c'est vous qui demandez explicitement au modèle de raisonner étape par étape. Depuis les modèles o1/o3 d'OpenAI (aujourd'hui fondus dans la gamme GPT-5.x, où un niveau d'effort de raisonnement réglable remplace des modèles séparés) et les derniers modèles Claude d'Anthropic, une partie des modèles génère aussi un **raisonnement natif** : ils réfléchissent en interne avant de répondre, sans qu'il soit nécessaire de le demander dans le prompt. Chez Anthropic, ce raisonnement adaptatif est même activé par défaut sur Claude Sonnet 5 et Claude Opus 5.

  Ce raisonnement natif ne rend pas les techniques de cette leçon inutiles : sur les modèles qui ne raisonnent pas nativement, ou quand vous voulez orienter le raisonnement dans une direction précise, le CoT prompté reste pertinent. Gardez aussi en tête que ce raisonnement interne consomme des tokens comme le reste de la réponse : il pèse donc sur le budget de la fenêtre de contexte du modèle.
</Note>
...
Essayez ce prompt avec et sans la phrase « Raisonnons étape par étape. ». Comparez les deux réponses : le résultat chiffré est-il identique ? Le raisonnement de la version CoT vous permet-il de repérer plus facilement une éventuelle erreur de calcul ?

## Testez vos connaissances

<Accordion title="Quiz : puisque Claude Sonnet 5 raisonne déjà nativement avant de répondre, demander « raisonnons étape par étape » dans le prompt devient-il inutile avec ce modèle ?">
  **Pas totalement.** Le raisonnement natif s'active automatiquement, mais il ne suit pas forcément la direction ou la méthode que vous souhaitez. Demander explicitement un raisonnement étape par étape reste utile pour orienter le modèle vers une décomposition précise (par exemple, imposer les critères d'évaluation d'un problème), plutôt que de laisser son raisonnement interne suivre son propre chemin.
</Accordion>
