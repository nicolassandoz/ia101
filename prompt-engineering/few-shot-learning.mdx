...
<Info>
  **Niveau** : Intermédiaire · **Prérequis** : [Bonnes pratiques du prompt engineering](/prompt-engineering/bonnes-pratiques) · **Temps de lecture** : ~5 min
</Info>
...
Plus vous fournissez d'exemples, plus vous contrôlez le format et la cohérence des réponses :

```mermaid
flowchart LR
    Z["Zero-shot (0 exemple)"] --> O["One-shot (1 exemple)"]
    O --> F["Few-shot (2 à 8 exemples)"]
    F --> Q["Cohérence et contrôle du format croissants"]
```
...
<Warning>
  Le modèle reproduit fidèlement tout ce que contiennent vos exemples, y compris leurs erreurs et leurs biais. Si un seul de vos exemples contient une classification fausse, une formulation biaisée ou une information incorrecte, le modèle a de bonnes chances de la reproduire avec la même assurance dans ses réponses suivantes. Relisez soigneusement chaque exemple avant de l'utiliser.
</Warning>
...
Testez par vous-même l'effet des exemples sur une tâche de classification.

```text
Tâche : Classe la priorité de ce ticket support (Urgent / Normal / Faible).

Ticket : "Bonjour, mon compte a été débité deux fois pour le même
abonnement ce matin, pouvez-vous vérifier ?"
Priorité :
```

Envoyez d'abord ce prompt seul (zero-shot), puis recommencez en ajoutant 3 exemples de tickets déjà classés avant votre question (few-shot). Comparez les deux réponses : la version few-shot est-elle plus courte et plus cohérente avec le format que vous auriez choisi vous-même ?

## Testez vos connaissances

<Accordion title="Quiz : pour améliorer un prompt few-shot qui ne donne pas de bons résultats, faut-il d'abord ajouter plus d'exemples ?">
  **Pas nécessairement.** Au-delà de 8 exemples, le gain marginal devient faible et le coût en tokens augmente. Il vaut mieux d'abord vérifier la **qualité** des exemples déjà présents : sont-ils représentatifs, suffisamment diversifiés, sans ambiguïté et cohérents dans leur format ? Trois exemples bien choisis surpassent systématiquement huit exemples redondants ou mal formulés.
</Accordion>
