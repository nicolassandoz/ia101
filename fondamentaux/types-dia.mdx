...
<Tabs>
  ...
  <Tab title="LLM">
    ...
    Le processus d'entraînement d'un assistant moderne peut comporter plusieurs phases :
    ...
    3. **Alignement sur des préférences** : le modèle peut être optimisé avec du RLHF, du RLAIF, du DPO ou d'autres méthodes afin d'améliorer son utilité et sa sécurité. Tous les fournisseurs n'utilisent pas exactement la même recette.
    ...
    En changeant d'échelle, les LLM progressent sur des tâches qui n'ont pas été enseignées explicitement : traduction, analogies, code ou résolution de problèmes. Certaines courbes de benchmark donnent l'impression d'une apparition soudaine. Une partie de cet effet dépend toutefois du choix de la métrique et du seuil de réussite. Parlez donc de **capacités émergentes** avec prudence : le phénomène est observé, mais son caractère réellement abrupt reste débattu.

    ### Les LLM les plus connus aujourd'hui

    | Modèle | Organisation | Particularité |
    |---|---|---|
    | **GPT-6 Astra** | OpenAI | Multimodal (texte, image, audio) |
    | **Claude Fable 5.1 / Opus 5.5 / Sonnet 5** | Anthropic | Contexte très long (1 million de tokens), plusieurs compromis coût-performance |
    | **Gemini 3.1 Pro** | Google DeepMind | Fenêtre de contexte d'1 million de tokens |
    | **Muse Spark 1.3** | Meta | Modèle phare propriétaire (poids fermés), orienté agents et code |
