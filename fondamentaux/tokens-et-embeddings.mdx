...
<Info>
  **Niveau** : Intermédiaire · **Prérequis** : [Les modèles de langage](/fondamentaux/modeles-de-langage) · **Temps de lecture** : ~5 min
</Info>
...
# Encodage générique utilisé par plusieurs générations de modèles OpenAI.
# Pour un calcul de coût en production, utilisez l'encodage exact indiqué
# dans la documentation du modèle ciblé.
...
# Les identifiants obtenus dépendent de l'encodage choisi.
```
...
**2. La limite de contexte** : chaque modèle a une **fenêtre de contexte maximale**, exprimée en tokens. GPT-6 Astra et Claude Sonnet 5 supportent chacun environ 1 million de tokens ; Claude Haiku 4.5 se limite à 200 000 tokens. Au-delà, le modèle ne peut plus "voir" les informations les plus anciennes de la conversation.
...
<Warning>
Ces ratios sont des approximations pratiques pour se faire une idée rapide, pas une valeur exacte. Ne les utilisez jamais pour un calcul de coût ou de dimensionnement critique en production : le découpage réel dépend du tokenizer propre à chaque modèle. Utilisez l'outil de tokenisation officiel de votre fournisseur (comme tiktoken pour OpenAI) avant de vous engager sur un budget ou une limite de contexte précise.
</Warning>

## Qu'est-ce qu'un embedding ?

Une fois le texte tokenisé, le modèle convertit les tokens en **vecteurs numériques** appelés *embeddings*. Concrètement, un embedding est une liste de nombres qui situe une unité de texte dans un espace mathématique à haute dimension. Les modèles d'embedding destinés à la recherche produisent aussi un vecteur pour une phrase ou un document entier.

Ce qui rend les embeddings puissants, c'est qu'ils **capturent des régularités sémantiques** : des textes utilisés dans des contextes similaires ont souvent des vecteurs proches. Attention, deux antonymes comme « chaud » et « froid » peuvent aussi être proches parce qu'ils partagent le même domaine.
...
Cet exemple historique illustre qu'une relation peut parfois apparaître géométriquement dans certains embeddings de mots. Ce n'est ni une loi générale, ni une opération qui fonctionne à l'identique avec tous les modèles modernes ou avec des embeddings de phrases.
...
- *« positif » et « négatif » peuvent être proches par domaine, malgré leur sens opposé*
...
<Accordion title="Quiz : Vrai ou Faux ? En français, 1 mot équivaut toujours à 1 token.">
  **Faux.** Un token n'est pas un mot. En français, on compte en moyenne environ 1,3 token par mot. Les mots très communs peuvent correspondre à 1 token, mais les mots longs, rares ou complexes (comme "anticonstitutionnellement") seront découpés en plusieurs tokens par le modèle.
</Accordion>
