...
<Info>
  **Niveau** : Avancé · **Prérequis** : [Tokens et embeddings](/fondamentaux/tokens-et-embeddings) · **Temps de lecture** : ~5 min
</Info>
...
Vu de haut, ce pipeline transforme un immense corpus brut en un assistant aligné :

```mermaid
flowchart LR
    A["Corpus massif de tokens"] --> B["Pré-entraînement"]
    B --> C["Modèle de base"]
    C --> D["Fine-tuning supervisé (SFT)"]
    D --> E["RLHF"]
    E --> F["Modèle aligné"]
    F --> G["Évaluation et déploiement"]
```
...
<Accordion title="Quiz : Le RLHF garantit-il que les réponses d'un LLM sont plus véridiques ?">
  **Non.** Le RLHF entraîne le modèle à maximiser un score de préférence donné par des annotateurs humains, c'est-à-dire à produire des réponses qui **plaisent**, pas nécessairement des réponses vraies. Un modèle peut ainsi apprendre qu'une réponse confiante et bien formulée est mieux notée qu'un aveu d'incertitude, même quand l'incertitude serait la réponse la plus honnête. C'est une des raisons pour lesquelles les LLM peuvent sembler très sûrs d'eux tout en se trompant, un phénomène détaillé dans la leçon suivante sur les hallucinations.
</Accordion>
