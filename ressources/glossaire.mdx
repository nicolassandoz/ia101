...
<AccordionGroup>
  ...
  <Accordion title="LLM (Large Language Model)">
    Un **LLM** (grand modèle de langage) est un modèle de réseau de neurones entraîné sur de vastes corpus pour traiter et générer du langage naturel. Les modèles actuels reposent majoritairement sur l'architecture Transformer. GPT-6 Astra, Claude Sonnet 5, Gemini 3.1 Pro, Muse Spark 1.3 (Meta), DeepSeek V4, Kimi K3 (Moonshot AI) et Grok 4.7 (xAI) comptent parmi les familles connues en septembre 2026.
  </Accordion>
...
<AccordionGroup>
  <Accordion title="Paramètre">
    Les **paramètres** d'un modèle sont les valeurs numériques internes (poids et biais) apprises pendant l'entraînement et qui déterminent son comportement. Leur nombre est parfois utilisé comme indicateur de capacité, mais il ne suffit pas pour comparer deux modèles : l'architecture, les données, l'entraînement et le nombre de paramètres activés à chaque requête comptent aussi. De nombreux fournisseurs de modèles propriétaires ne publient pas ce chiffre.
  </Accordion>
