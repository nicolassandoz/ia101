...
<AccordionGroup>
  <Accordion title="ACV (Analyse de cycle de vie)">
    L'**analyse de cycle de vie** est une méthode normalisée d'évaluation des impacts environnementaux d'un produit ou d'un service sur toute son existence : extraction des matières premières, fabrication, usage, fin de vie. Appliquée à l'IA, elle couvre la fabrication du matériel, la construction des centres de données, l'entraînement du modèle et l'inférence. C'est la seule approche qui évite de déplacer un impact d'une étape à une autre sans le voir.
  </Accordion>
  ...
  <Accordion title="AI Act">
    L'**AI Act** (règlement (UE) 2024/1689) est le règlement européen sur l'intelligence artificielle, entré en vigueur le 1<sup>er</sup> août 2024 et modifié en juillet 2026 par le *Digital Omnibus on AI*. Il classe les systèmes en quatre niveaux de risque (inacceptable, haut, limité, minimal) et impose des obligations proportionnées. Les règles de transparence et le régime de sanctions s'appliquent depuis le 2 août 2026 ; celles des systèmes à haut risque à partir de décembre 2027.
  </Accordion>
  ...
  <Accordion title="Biais d'automatisation">
    Le **biais d'automatisation** est la tendance à accepter sans contrôle la recommandation d'un système automatisé, et à ignorer des informations contradictoires pourtant disponibles. Documenté bien avant l'IA générative dans l'aviation et la médecine, il s'accentue avec la fatigue, la charge de travail et, paradoxalement, avec la fiabilité habituelle du système. C'est le principal risque cognitif de l'usage quotidien des assistants IA.
  </Accordion>
...
<AccordionGroup>
  <Accordion title="Dette cognitive">
    La **dette cognitive** désigne le déficit d'apprentissage et de mémorisation qui accompagne la délégation d'une tâche intellectuelle à un outil : on gagne du temps immédiatement, au prix d'un encodage plus superficiel. Le terme provient d'une étude du MIT Media Lab (2025) sur la rédaction assistée par IA. À manier avec ses limites : échantillon réduit, prépublication, une seule tâche étudiée.
  </Accordion>

  <Accordion title="Effet rebond">
    L'**effet rebond** (ou paradoxe de Jevons) décrit le phénomène par lequel les gains d'efficacité d'une technologie sont absorbés, puis dépassés, par l'augmentation des usages qu'ils rendent possible. Appliqué à l'IA : le coût énergétique par requête chute rapidement, mais le nombre de requêtes croît plus vite encore. C'est pourquoi l'efficacité technique seule ne suffit pas à réduire l'impact global.
  </Accordion>
  ...
  <Accordion title="Foundation model">
    Un **foundation model** (modèle de fondation) est un grand modèle entraîné sur des quantités massives de données générales, conçu pour être adapté à une multitude de tâches en aval. GPT-6 Astra, Claude, Gemini, Muse Spark, DeepSeek, Kimi K3 ou Qwen3.8-Max sont des exemples de foundation models. Leur polyvalence les distingue des modèles spécialisés entraînés pour une unique tâche précise.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="IA frugale">
    L'**IA frugale** désigne une approche visant à obtenir le résultat attendu avec le minimum de ressources : modèle le plus petit possible, contexte minimalement suffisant, recours justifié à l'IA générative. L'AFNOR SPEC 2314 (2024) en fournit un référentiel français : méthodologie de mesure par analyse de cycle de vie, 31 fiches de bonnes pratiques et règles encadrant les allégations de frugalité.
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="LLM (Large Language Model)">
    Un **LLM** (grand modèle de langage) est un modèle de réseau de neurones entraîné sur des corpus de texte massifs pour comprendre et générer du langage naturel. Ils sont basés sur l'architecture Transformer et comptent des milliards voire des billions de paramètres. GPT-6 Astra, Claude Sonnet 5, Gemini 3.1 Pro, Muse Spark (Meta), DeepSeek V4, Kimi K3 (Moonshot AI), Qwen3.8-Max (Alibaba) et Grok 4.6 (SpaceXAI, ex-xAI) sont parmi les LLM les plus connus.
  </Accordion>
  ...
  <Accordion title="Multi-modal">
    Un modèle **multi-modal** est capable de traiter et de générer plusieurs types de données simultanément, comme le texte, les images, l'audio ou la vidéo. GPT-6 Astra, Gemini et Claude Sonnet 5 sont des exemples de LLM multi-modaux. Cette capacité ouvre des cas d'usage comme l'analyse d'images, la transcription audio ou la génération de contenu visuel à partir de descriptions textuelles.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="Paramètre">
    Les **paramètres** d'un modèle sont les valeurs numériques internes (poids et biais) appris pendant la phase d'entraînement et qui définissent son comportement. Un modèle comme GPT-6 Astra en contient des centaines de milliards. Le nombre de paramètres est souvent utilisé comme indicateur approximatif de la capacité d'un modèle, même si ce n'est pas le seul facteur déterminant de ses performances.
  </Accordion>
  ...
  <Accordion title="PUE / WUE">
    Le **PUE** (*Power Usage Effectiveness*) mesure l'efficacité énergétique d'un centre de données : le rapport entre l'énergie totale consommée et celle réellement utilisée par les équipements informatiques. Un PUE de 1,1 est excellent, 2,0 médiocre. Le **WUE** (*Water Usage Effectiveness*) mesure la consommation d'eau par unité d'énergie informatique. Les deux s'arbitrent : un refroidissement évaporatif améliore le PUE en dégradant le WUE. Juger un centre de données sur le seul PUE est donc trompeur.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="Scope 1, 2, 3">
    Les **scopes** classent les émissions de gaz à effet de serre d'une organisation : le **scope 1** couvre les émissions directes, le **scope 2** l'électricité achetée, et le **scope 3** l'ensemble de la chaîne de valeur (achats, matériel, services cloud, déplacements). L'usage de l'IA relève principalement du scope 2 (électricité si vous hébergez) et du scope 3 (services d'IA achetés, matériel). C'est là qu'il s'intègre au bilan carbone, plutôt que dans une démarche séparée.
  </Accordion>
</AccordionGroup>
