...
<Info>
  **Niveau** : Intermédiaire · **Prérequis** : [Modèles de langage](/fondamentaux/modeles-de-langage) · **Temps de lecture** : ~4 min
</Info>
...
```mermaid
flowchart TD
    S["Instructions système"] --> CTX["Fenêtre de contexte"]
    H["Historique des messages"] --> CTX
    D["Données injectées"] --> CTX
    U["Message utilisateur actuel"] --> CTX
    CTX --> M["LLM"]
    M --> R["Réponse"]
```
...
<Accordion title="Quiz : Un modèle dispose d'une fenêtre de contexte d'un million de tokens. Est-ce une bonne idée d'y injecter systématiquement tous les documents disponibles, même les non pertinents, puisque la place ne manque pas ?">
